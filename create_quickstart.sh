...
su - zato -c"$ZATO_BIN quickstart create $ZATO_ENV mysql localhost 6379 --kvdb_password '' --odb_host $ZATO_ODBHOST_ENV --odb_port $ZATO_ODBPORT_ENV --odb_user $ZATO_ODBUSER_ENV --odb_db_name $ZATO_ODBDATABASE_ENV --odb_password $ZATO_ODBSECRET_ENV --cluster_name $ZATO_CLUSTERNAME --servers 2 $ZATO_ADDITIONALPARAM"
